...
const state = {
  colors: { top: '#ffffff', bottom: '#ffffff' },
  infoOpen: false,
};
...
function createHeroArticle(item, index) {
...
function createPortfolioArticle(item, index) {
  const article = document.createElement('article');
...
function renderPortfolio() {
  ...
  root.appendChild(createSiteTitle());

  MEDIA.forEach((item, index) => {
    const article = index === 0 ? createHeroArticle(item, index) : createPortfolioArticle(item, index);
    if (index === 0) {
